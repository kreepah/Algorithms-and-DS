...
def database_search(database,field,value):
    result = []
    if (field == 'name'):
        hashSlot = hash_function(value)
        for i in range(0, len(database.name_table[hashSlot])):
            Ndata = database.name_table[hashSlot]
            if (Ndata[i].name == value):
                result.append(Ndata[i])
    elif (field == 'shape'):
        hashSlot = hash_function(value)
        for j in range(0, len(database.shape_table[hashSlot])):
            Sdata = database.shape_table[hashSlot]
            if (Sdata[j].shape == value):
                result.append(Sdata[j])
    else:
        hashSlot = hash_function(value)
        for k in range(0, len(database.color_table[hashSlot])):
            Cdata = database.color_table[hashSlot]
            if (Cdata[k].color == value):
                result.append(Cdata[k])
    return result
    pass
...
def database_delete(database,field,value):
    robot = database_search(database, field, value)
    for i in range (0, len(robot)):
        n_slot = hash_function(robot[i].name)
        s_slot = hash_function(robot[i].shape)
        c_slot = hash_function(robot[i].color)
        database.name_table[n_slot].remove(robot[i])
        database.shape_table[s_slot].remove(robot[i])
        database.color_table[c_slot].remove(robot[i])
