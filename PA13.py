...
def min_heap_insert(A, key):
    A.append(key)
    heap_decrease_key (A, (len(A)-1), key.get_dist())
...
def min_heapify(A, i):
    L = (2 * i) + 1
    R = (2 * i) + 2
    if (L < (len(A)) and A[L].get_dist() < A[i].get_dist()):
        smallest = L
    else:
        smallest = i
    if (R < (len(A)) and A[R].get_dist() < A[smallest].get_dist()):
        smallest = R
    if (smallest != i):
        temp = A[i]
        A[i] = A[smallest]
...
def heap_decrease_key(A,i,key):
    ...
    if (i < 2):
        ...
        parent = int (math.log((i-1),2))
    while (i > 0 and (A[parent].get_dist() > A[i].get_dist())):
        temp = A[i]
        A[i] = A[parent]
        A[parent] = temp
        i = parent
...
def relax(u,v, jump_matrix, Q):
    if (v.get_dist() > (u.get_dist() + jump_matrix[u.index][v.index])):
        v.set_dist(u.get_dist() + jump_matrix[u.index][v.index])
        i = Q.index(v)
...
def hyperspace_jump_route(star_list,jump_matrix):
    for i in star_list:
        if (i.color == "red"):
            ...
            i.set_dist(float('inf'))
            
    start = star_list [0]
    goal = star_list [1]
    path = [] 
    Q = build_min_heap (star_list)    
    while (len (Q) > 0):
        u = heap_extract_min (Q)
        for v in u.adj:
            relax (u, v, jump_matrix, Q)

    node = goal.prev
    path.insert (0,goal)
    while ((node != None) and (node.color != "red")):
        path.insert (0, node)
        node = node.prev
    path.insert (0, start)    
