...
def max_heapify(task_heap, i):
    L = (2 * i) + 1
    R = (2 * i) + 2
    if (L < (len(task_heap)) and task_heap[L].priority > task_heap[i].priority):
        largest = L
    else:
        largest = i
...
def increase_task_priority(task_heap,i,new_priority):
    ...
    #    print ("new key smaller than old key")
         return
    ...
    while (i > 0 and (task_heap[parent].priority < task_heap[i].priority)):
        temp = task_heap[i]
        task_heap[i] = task_heap[parent]
        task_heap[parent] = temp
        i = parent
        if (i < 2):
            parent = 0
        else:
