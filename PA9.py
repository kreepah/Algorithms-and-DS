...
def optimize_wings(price_list,num_wings):
    rev = [0] * num_wings
    split = [0] * num_wings
    opt = [Order(0,0)]* num_wings
    rev[0] = 0
    for j in range (0, num_wings):
        best = float('inf')
        for i in range (0, len(price_list)):
            if (best > (price_list[i].price + rev[j - i])):
                best = price_list[i].price + rev[j - i]
                split[j] = price_list[i].wings
        rev[j] = best

    k = num_wings 
    x = 0
    while (k > 0):
        opt[x].wings = split[k - 1]
        opt[x].price = rev[k - 1]
        k = k - split[k - 1]
        x = x+1
    
    return opt  #Placeholder return value to avoid errors
